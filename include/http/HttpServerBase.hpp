...
#ifndef _APN_HTTP_HTTPSERVERBASE_HPP_
#define _APN_HTTP_HTTPSERVERBASE_HPP_
...
public:
	...
	std::unordered_map<std::string, std::unordered_map<std::string,
	    std::function<void(typename HttpServerBase<socket_type>::Response&, std::shared_ptr<typename HttpServerBase<socket_type>::Request>)> > >  resource;
	std::unordered_map<std::string,
	    std::function<void(typename HttpServerBase<socket_type>::Response&, std::shared_ptr<typename HttpServerBase<socket_type>::Request>)> > default_resource;
private:
	std::vector<std::pair<std::string, std::vector<std::pair<std::regex,
	    std::function<void(typename HttpServerBase<socket_type>::Response&, std::shared_ptr<typename HttpServerBase<socket_type>::Request>)> > > > > opt_resource;
public:
	void start()
	{
...
protected:
	...
	HttpServerBase(unsigned short port, size_t num_threads, size_t timeout_request, size_t timeout_send_or_receive) :
	...
	HttpServerBase(std::string address, unsigned short port, size_t num_threads, size_t timeout_request, size_t timeout_send_or_receive) :
		num_threads(num_threads), timeout_request(timeout_request), timeout_content(timeout_send_or_receive)
	{
		// endpoint(boost::asio::ip::tcp::v4(), port), acceptor(io_service, endpoint),
		boost::asio::ip::tcp::resolver resolver(io_service);
		boost::asio::ip::tcp::resolver::query query(address, std::to_string(port).c_str());
	...
	void write_response(std::shared_ptr<socket_type> socket, std::shared_ptr<Request> request,
	                    std::function<void(typename HttpServerBase<socket_type>::Response&, std::shared_ptr<typename HttpServerBase<socket_type>::Request>)>& resource_function)
	{
		//Set timeout on the following boost::asio::async-read or write function
		std::shared_ptr<boost::asio::deadline_timer> timer;
		if(timeout_content>0)
...
#endif	/* _APN_HTTP_HTTPSERVERBASE_HPP_ */
